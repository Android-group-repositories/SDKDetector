...
def InsertAppInfo(db):
    f = open('./res.json', encoding='utf-8')
    res = json.load(f)
    cnt =1
    for pkg in res.keys():
        for verstion in res[pkg].keys() :
            for appPath in  res[pkg][verstion] :
                appHash = appPath.split('/')[-1][:-4]
                
                data = (appHash,pkg,verstion)
                
                cnt = cnt +1
                db.insert("INSERT INTO appInfo VALUES (NULL,?,?,?)",data) 
    print(str(cnt))
    # print(res['air.com.oak.OddSocksMob'])
